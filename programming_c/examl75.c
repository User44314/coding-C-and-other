...
int stack_height = 0; // stack_height is used to store the height of the stack

void panic(const char *msg) // panic function is used to print the error message and exit the program
{
	fprintf(stderr, "%s\n", msg);
	exit(EXIT_FAILURE);
}

void push(double value) // push function is used to push the value to the stack
{
...
double pop(void) // pop function is used to pop the value from the stack
{
	if (stack_height == 0)
...
{
	int i;
	double value; // double type is used to store the value
