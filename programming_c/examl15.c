/******************************************************
  ...
  for matching {} () "" '' [] pairs. Don't forget about
  comments.
******************************************************/
#include <stdio.h>
...
int getline(void); /* read a line into line, return length */
...
int main()
{
  ...
  while ((len = getline()) > 0)
  {
    ...
    while (t < len)
    {
      if (line[t] == '[')
      {
        brace++; // increment the brace count
      }
      if (line[t] == ']')
      {
        brace--; // decrement the brace count
      }
      if (line[t] == '(')
      {
        parenthesis++;
      ...
      if (line[t] == '\'')
      {
        s_quote *= -1; // toggle the single quote mark
      }
      if (line[t] == '"')
      {
        d_quote *= -1;
