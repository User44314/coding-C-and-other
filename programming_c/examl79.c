...
#define MAXLINES 1024 /* max #lines to be sorted */
#define MAXINPUT 10240 /* max input size */
#define DEF_NUM_LINES 10 /* default number of lines to print */

int getlines(char *); /* get lines from input */
void parse_args(int, char **); /* parse command line arguments */

char linestr[MAXINPUT]; /* buffer for input lines */
char *lineptr[MAXLINES]; /* pointers to text lines */

int num_of_lines = DEF_NUM_LINES; 	/* number of lines to print */

int main(int argc, char *argv[])
{
	int lines; /* number of lines read */
	int ltp = 0; /* lines to print */
	int i; 
	int c;
